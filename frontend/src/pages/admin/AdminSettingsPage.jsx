...
import {
  ...
  Globe, Info, Shield, HelpCircle, Link, Music
} from 'lucide-react';
import { getAdminSettings, saveAdminSettings } from '@/services/admin.service';
import ShinobiThemeAudioTab from '@/components/admin/ShinobiThemeAudioTab';
import toast from 'react-hot-toast';

const VALID_SETTINGS_TABS = ['general', 'security', 'ai', 'storage', 'features', 'audio'];

export default function AdminSettingsPage() {
  const [searchParams, setSearchParams] = useSearchParams();
  const tabFromQuery = searchParams.get('tab');
  const [activeTab, setActiveTab] = useState(() =>
    VALID_SETTINGS_TABS.includes(tabFromQuery) ? tabFromQuery : 'general'
  );

  useEffect(() => {
    const currentQueryTab = searchParams.get('tab');
    if (currentQueryTab && VALID_SETTINGS_TABS.includes(currentQueryTab)) {
        ...
        <form onSubmit={handleSave} className="grid grid-cols-1 lg:grid-cols-4 gap-6 items-start">
          ...
          <div className="space-y-2 lg:sticky lg:top-4">
            ...
            <button
              ...
              className={`w-full text-left px-4 py-3 rounded-xl border flex items-center gap-2.5 transition-all text-xs font-semibold
                ${activeTab === 'audio'
                  ? 'bg-brand-600/15 border-brand-500/40 text-white shadow-lg ring-1 ring-brand-500/30'
                  : 'bg-[#0f0f22]/30 border-white/[0.06] text-slate-400 hover:border-white/10 hover:text-slate-200'}`}
            >
              <Music size={14} className="text-brand-400" /> Shinobi Audio (忍音)
            </button>
          ...
          <div className="lg:col-span-3 card p-6 bg-[#0f0f22]/30 border-white/[0.06] space-y-6">
            ...
            {activeTab === 'audio' && (
              <ShinobiThemeAudioTab
                themeSounds={form.themeSounds || {}}
